CREATE PROCEDURE [dbo].[emk5_AddMedRecordInstrum]
@IdCaseMis INT
AS
BEGIN
	...
	,MOT.MOTCONSU_ID AS IdCaseMis
		--/medRecord	 	1..1	 	Медицинская запись. Для значения xsi:type указывается соответствующий тип наследуемого объекта. Описание типов объекта приведено в разделе "Тип MedRecord"
					--/ConsultNote
						...
						--Attachment	0..1	DocumentAttachment	Неструктурированное (бинарное) содержание документа
							...
							--Data	0..1	base64Binary	Cтруктура SignData в формате base64binary
								--SignData Если передается неподписанный документ, то поля PublicKey, Hash и Sign не заполняются.
									...
									,(SELECT sd.BODY FROM SIGN_DATA AS sd WHERE sd.MOTCONSU_ID = MOT.MOTCONSU_ID AND sd.SIGN_ROLE = 'ORG' AND sd.SIGN_TYPE = 'INSTRUMENTAL')as Base64binary
									--,MOT.MSE_BodyPreparedReport as Base64binary
									,(SELECT sd.SIGN_BODY FROM SIGN_DATA AS sd WHERE sd.MOTCONSU_ID = MOT.MOTCONSU_ID AND sd.SIGN_ROLE = 'ORG' AND sd.SIGN_TYPE = 'INSTRUMENTAL')as OrganizationSign
									--,MOT.MSE_ECP_SIGN_DATA_ORG as OrganizationSign
									,(SELECT sd.SIGN_BODY FROM SIGN_DATA AS sd WHERE sd.MOTCONSU_ID = MOT.MOTCONSU_ID AND sd.SIGN_ROLE = 'DOCTOR' AND sd.SIGN_TYPE = 'INSTRUMENTAL')as PersonalSigns
						...
						,(CASE WHEN isnull(DOC.Description, '') = '' THEN 'Инструментальное исследование ' + convert(varchar(19), MOT.DATE_CONSULTATION, 121) ELSE DOC.Description END) AS Header
						--,napr.VID_NAPR IdReferralType
						,instrum.DATE_CONSULTATION [Date]
						, CASE 
						    WHEN instrum.METOD_ISSLED = 1 THEN 10
						    WHEN instrum.METOD_ISSLED = 2 THEN 70
						    WHEN instrum.METOD_ISSLED = 3 THEN 23
						    WHEN instrum.METOD_ISSLED = 4 THEN 1
						    WHEN instrum.METOD_ISSLED = 5 THEN 160
						    WHEN instrum.METOD_ISSLED = 6 THEN 150
						    WHEN instrum.METOD_ISSLED = 7 THEN 7
						    ELSE 9999 END  [Type]
						, 1 [Priority]
						,instrum.ZAKLYUCHENIE [Text]
						--,case when napr.VID_NAPR = 1 then 2 ELSE NULL END  HospitalizationOrder
						--,napr.KOD_DIAGNOZA MkbCode
						--,fo.oid
...
WHERE instrum.MOTCONSU_ID = @IdCaseMis
END

GO


GRANT CONTROL ON [dbo].[emk5_AddMedRecordInstrum] TO [iemk_user]

GRANT ALTER ON [dbo].[emk5_AddMedRecordInstrum] TO [iemk_user]

GRANT EXECUTE ON [dbo].[emk5_AddMedRecordInstrum] TO [iemk_user]
