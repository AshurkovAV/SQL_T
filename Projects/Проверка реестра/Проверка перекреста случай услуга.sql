...
JOIN (SELECT u.PROFIL, u.DateEnd, u.DateStart, pv.FAM, pv.IM, pv.OT, pv.DR, pv.Doctor 
      ...
      WHERE pv.ReestrMedOrgID = @ReestrMedOrgID AND u.CodUslugi in ('B04.031.002', 'B04.023.002', 'B04.029.002', 'B04.010.002', 'B04.053.004', 'B04.010.002', 'B04.053.002',
      'B04.064.002', 'B04.058.003', 'B04.028.002', 'B04.001.002', 'B04.050.002', 'B01.047.005', 'B04.023.002')) as u
      ON pv.ReestrMedOrgID = @ReestrMedOrgID
...
--DELETE FROM PacientVisit WHERE id IN (SELECT ID FROM @t) 

SELECT ID, FAM, IM, OT, CAST(DR AS DATE)DR, Doctor, CAST(HelpStart AS DATE)HelpStart, CAST(DateStart AS DATE)DateStart, 
CAST(HelpEnd AS DATE)HelpEnd, CAST(DateEnd AS DATE)DateEnd,
	--(SELECT f.nam_smok
 --    FROM f002 f WHERE f.smocod = t.smo),
'Перекрест случай с услугой' AS com FROM @t t
ORDER BY Doctor
