...
--DECLARE @R_YEAR INT = 2016
DECLARE @ReestrMedOrgID INT = 5126
DECLARE @SCHET_ID INT = 2059
...
INNER JOIN SCHET AS s ON s.ID = sl.SCHET_ID AND s.ID = @SCHET_ID
...
AND CAST(pv.HelpStart AS DATE) BETWEEN CAST(dateadd(day,1,stac.DATE_1) AS DATE) AND CAST(stac.DATE_2 AS DATE)
