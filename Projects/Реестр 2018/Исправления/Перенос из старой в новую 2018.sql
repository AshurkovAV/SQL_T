...
join ReestrMedOrgDb r on pv.ReestrMedOrgID = r.ID AND r.ID = 5127
join SCHET sc on sc.ID = @sc
WHERE pv.COMENTP is null

INSERT INTO [dbo].[D3_ZSL] ([VIDPOM],[NPR_MO],[RSLT],[ISHOD]
 ,[FOR_POM]
 ,[DATE_Z_1],[DATE_Z_2]
,[OS_SLUCH_REGION],[IDSP], LPU
,[D3_SCID], [D3_PID], ZSL_ID
)
SELECT TypeHelp, [NPR_MO], HelpResult, HelpIshod ,
CASE WHEN TypeSluch = 3 THEN  '2' ELSE 3 END,
HelpStart, HelpEnd,
