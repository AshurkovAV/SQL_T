...
Изменил ashur 19.04.2021 добавил AND mot.MEDDEP_ID = m.MEDDEP_ID
30.01.2022 ashur добавление инструментального исследования, направлене 057 форма
--Изменения ashur 22.03.2022 в IdPosition поменяли должность SPEC.DOLZGNOST на SPEC.CODE_NSI_DOLZGNOST
*/
ALTER VIEW [dbo].[view_emk3_GetAddCase]
AS

SELECT      CASE WHEN instrum.DATA_W438_PROTOKOL_ISSLED_ID IS NOT NULL THEN instrum.DATE_CONSULTATION
             ...
             ELSE
                  CAST ((SELECT        TOP (1) EV_DATE_CLOSE
                                 ...
                                 MOT.MOTCONSU_ID AS HistoryNumber, 
                                 --CASE 
							--WHEN napr057.DATA_W438_NAPR_NA_GOSP_ID    IS NOT NULL THEN napr057.KRN_GUID 
							--WHEN instrum.DATA_W438_PROTOKOL_ISSLED_ID IS NOT NULL THEN instrum.KRN_GUID
							--WHEN epikriz.DATA_W438_EPIK_ZAK_AMB_ID    IS NOT NULL THEN epikriz.KRN_GUID ELSE MOT.MOTCONSU_ID END IdCaseMis,                                
                               MOT.MOTCONSU_ID AS IdCaseMis , MOT.MOTCONSU_ID, 
				  ...
				  ELSE '6' END) 
					...
					ELSE ETF.ED_TREATMENT_FORM_CODE END) END AS IdCaseAidType, 1 AS IdPaymentType, 2 AS Confidentiality, 2 AS DoctorConfidentiality, 2 AS CuratorConfidentiality,
                             (SELECT        TOP (1) IEMK_OID
                               FROM            dbo.FM_ORG
                               WHERE        (FM_ORG_ID = 1)) AS IdLpu, (CASE WHEN ISNULL(DAPC.ED_OUTCOMES_ID, '') = '' and ISNULL(DAPC.OMS_V012_ID, '') != ''
      THEN (CASE WHEN DAPC.OMS_V012_ID in (1,5,9) THEN 1
      WHEN DAPC.OMS_V012_ID in (2,6,11,15) THEN 2
      WHEN DAPC.OMS_V012_ID in (3,7,12,16,10) THEN 3
      WHEN DAPC.OMS_V012_ID in (4,8,13,17) THEN 4
      WHEN DAPC.OMS_V012_ID in (14) THEN 5
      END) ELSE 
      (CASE WHEN ISNULL(DAPC.ED_OUTCOMES_ID, '') = '' THEN 3 ELSE DAPC.ED_OUTCOMES_ID END)
      END) AS IdCaseResult, MOT.REC_NAME AS Comment, 
			...
			(CASE WHEN DAPC.ED_MED_PROVISION_CONDITIONS_ID = 3 OR DAPC.ED_MED_PROVISION_CONDITIONS_ID = 4 THEN 2
				...
				END) AS IdCaseType,  --если пусто то ставим код 2 Амбулаторный случай
                         ...
                         DAPC.ED_OUTCOMES_ID = 5 THEN 3 ELSE 17 END) AS IdAmbResult, 1 AS IsActive,
                             fm.depart_oid  AS CaseDto_DoctorInCharge_IdLpu, SPEC.NSI_SPECIALISATION_ID AS CaseDto_DoctorInCharge_IdSpeciality, SPEC.CODE_NSI_DOLZGNOST AS CaseDto_DoctorInCharge_IdPosition, 
                         ...
                         223 AS Person_Documents_IdentityDocument_IdDocumentType, MED.SYS_SNILS AS Person_Documents_IdentityDocument_DocN, 'ПФР' AS Person_Documents_IdentityDocument_ProviderName,
                             fm.depart_oid  AS Authenticator_Doctor_IdLpu, SPEC.NSI_SPECIALISATION_ID AS Authenticator_Doctor_IdSpeciality, SPEC.CODE_NSI_DOLZGNOST AS Authenticator_Doctor_IdPosition, 
                         ...
                         'ПФР' AS Doctor_Person_Documents_IdentityDocument_ProviderName, 4 AS IdRole,
                             fm.depart_oid  AS Author_Doctor_IdLpu, SPEC.NSI_SPECIALISATION_ID AS Author_Doctor_IdSpeciality, SPEC.CODE_NSI_DOLZGNOST AS Author_Doctor_IdPosition, 
                         ...
                         223 AS Author_Doctor_Person_IdDocumentType, MED.SYS_SNILS AS Author_Doctor_Person_DocN, 'ПФР' AS Author_Doctor_Person_ProviderName,
                             fm.depart_oid  AS LegalAuthenticator_Doctor_IdLpu, SPEC.NSI_SPECIALISATION_ID AS LegalAuthenticator_Doctor_IdSpeciality, SPEC.CODE_NSI_DOLZGNOST AS LegalAuthenticator_Doctor_IdPosition, 
                         ...
                         SPEC.CODE_NSI_DOLZGNOST AS StepAmb_Doctor_IdPosition, MED.DATA_ROGDENIQ AS StepAmb_Doctor_BirthDate, MED.KRN_GUID AS StepAmb_Doctor_IdPersonMis, MED.NOM AS StepAmb_Doctor_FamilyName,
                         ...
                         CASE 
							WHEN napr057.DATA_W438_NAPR_NA_GOSP_ID    IS NOT NULL THEN 'NAPR057' 
							WHEN instrum.DATA_W438_PROTOKOL_ISSLED_ID IS NOT NULL THEN 'INSTRUMENTAL'
							WHEN epikriz.DATA_W438_EPIK_ZAK_AMB_ID    IS NOT NULL THEN 'EPIKRIZ' 
							WHEN llo.DATA236_ID                       IS NOT NULL THEN 'LLO'ELSE NULL END typeDoc
FROM            dbo.MOTCONSU AS MOT WITH (NOLOCK) LEFT OUTER JOIN
                         ...
                         DATA_W438_NAPR_NA_GOSP napr057 ON napr057.MOTCONSU_ID = MOT.MOTCONSU_ID LEFT JOIN
                         DATA_W438_EPIK_ZAK_AMB epikriz ON epikriz.MOTCONSU_ID = MOT.MOTCONSU_ID LEFT JOIN
                         DATA_GRACE_PRESCRIPTION llo ON mot.MOTCONSU_ID = llo.MOTCONSU_ID LEFT JOIN 
                          DATA_W438_MSE_NAPRAV napr ON napr.SLUCHAI_ZABOL = MOT.MOTCONSU_ID LEFT JOIN
                          DATA_W438_PROTOKOL_ISSLED instrum ON instrum.MOTCONSU_ID = MOT.MOTCONSU_ID   LEFT JOIN
