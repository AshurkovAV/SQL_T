		...
		--/medRecord	 	1..1	 	Медицинская запись. Для значения xsi:type указывается соответствующий тип наследуемого объекта. Описание типов объекта приведено в разделе "Тип MedRecord"
					--/DiagnosisInfo	1..1	DiagnosisInfo	Диагноз
						...
						,CASE WHEN instrum.DATA_W438_PROTOKOL_ISSLED_ID IS NOT NULL THEN cast(instrum.DATE_CONSULTATION as DATE)
						      WHEN epikriz.DATA_W438_EPIK_ZAK_AMB_ID    IS NOT NULL THEN cast(epikriz.DATE_CONSULTATION as DATE)
						      WHEN llo.DATA236_ID                       IS NOT NULL THEN cast(llo.DATE_CONSULTATION	as DATE)					      
						...
						,CASE WHEN instrum.DATA_W438_PROTOKOL_ISSLED_ID IS NOT NULL THEN instrum.PROTOKOL
						      WHEN epikriz.DATA_W438_EPIK_ZAK_AMB_ID    IS NOT NULL THEN epikriz.MKB_SLUCHAYA
						      WHEN llo.DATA236_ID                       IS NOT NULL THEN ISNULL(llo.LEKARSTVENNAQ_FORMA, '') + ' ' + ISNULL(llo.DOZIROVKA, '') + ' ' + ISNULL(llo.OTPUSK_APTEKA, '')
             ...
             ELSE MKB.CODE END AS MkbCode
				...
				--/Doctor	1..1	MedicalStaff	Сведения о медицинском работнике
					...
					,SPEC.CODE_NSI_DOLZGNOST as IdPosition
				--/Doctor
