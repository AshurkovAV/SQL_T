...
,(SELECT v002.NameWithID FROM v002 WHERE Id = PROFIL)profil													
from 													
(select distinct --sank.id sank,sank.S_OSN, 													
zs.IDSP,zs.PR_NOV,													
sch.code_mo,sch.MONTH,zs.id,---s.NHISTORY,													
s.sum_m SUMV,sank.id sank,sank.s_osn s_osn,v.UMPNAME usl_ok, s.PROFIL,													
CASE 													
...
FROM [D3_SCHET_OMS] sch													
		...
		where sch.year=2018									
		AND sch.[MONTH] IN (10,11,12)		AND sch.									
		...
		group by code_mo,											
		month,											
		t.usl_ok+'  '+t.tip+' ' +isnull(nam,''), PROFIL										
