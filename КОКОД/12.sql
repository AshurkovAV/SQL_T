...
Insert @temp_ksg
select sl.USL_OK, tf.D3_ZSLID, tf.ID, vm.ID vm_id, ksg.NKSG, t.ID as IDKSG, ksg.USL, ksg.DS1, ksg.DS2, ksg.VOZR as VOZR, ksg.POL, ksg.DLIT, ksg.[DOP_KR], ksg.DFR,
	d3_ksg.N_KSG tf_idksg, tf.DS1 tf_ds, ds2.ds sl_ds2, vm.VID_VME vm_vid_vme, tf.kd tf_kday, tf.sum_m tf_sumv_usl, tf.tarif tf_tarif, sl.sumv sl_sumv, tf.podr tf_podr,-- tf.DATE_1 tf_date_in, tf.DATE_2 tf_date_out, 
						KDAY = 
							(CASE 
							  WHEN sl.USL_OK = 1 THEN (CASE WHEN tf.DATE_1 = tf.DATE_2 THEN 1 ELSE DATEDIFF(DAY, tf.DATE_1, tf.DATE_2) END)
                            ...
                            (CASE 
								WHEN (sl.RSLT in (102, 105, 106, 107, 108, 110)) THEN 
                            		(CASE
									    ...
									    WHEN d3_ksg.N_KSG in ('1634', '1655', '1656', 'st13.003') and DATEDIFF(DAY, tf.DATE_1, tf.DATE_2) >= 4
											THEN ROUND(t2.TARIF*ISNULL(upr.UPR,1.00)*(CASE WHEN d3_ksg.N_KSG in (select [IDKSG] from [dbo].[CalNotKus] Where tf.DATE_2 >= TBEG and tf.DATE_2 < TEND +1) THEN 1.00 ELSE ISNULL(kf.KUS,1.00) END)*ISNULL(d3_ksg.IT_SL,1.00) * 1.00, 2)

                            			WHEN DATEDIFF(DAY, tf.DATE_1, tf.DATE_2) < 4
											THEN ROUND(t2.TARIF*ISNULL(upr.UPR,1.00)*(CASE WHEN d3_ksg.N_KSG in (select [IDKSG] from [dbo].[CalNotKus] Where tf.DATE_2 >= TBEG and tf.DATE_2 < TEND +1) THEN 1.00 ELSE ISNULL(kf.KUS,1.00) END)*ISNULL(d3_ksg.IT_SL,1.00) * 0.20, 2)
										ELSE ROUND(t2.TARIF*ISNULL(upr.UPR,1.00)*(CASE WHEN d3_ksg.N_KSG in (select [IDKSG] from [dbo].[CalNotKus] Where tf.DATE_2 >= TBEG and tf.DATE_2 < TEND +1) THEN 1.00 ELSE ISNULL(kf.KUS,1.00) END)*ISNULL(d3_ksg.IT_SL,1.00) * 0.50, 2)
									END)
								WHEN (sl.RSLT not in (102, 105, 106, 107, 108, 110)) THEN 
                            		(CASE 
                            			...
                            			WHEN DATEDIFF(DAY, tf.DATE_1, tf.DATE_2) < 4
											THEN ROUND(t2.TARIF*ISNULL(upr.UPR,1.00)*(CASE WHEN d3_ksg.N_KSG in (select [IDKSG] from [dbo].[CalNotKus] Where tf.DATE_2 >= TBEG and tf.DATE_2 < TEND +1) THEN 1.00 ELSE ISNULL(kf.KUS,1.00) END)*ISNULL(d3_ksg.IT_SL,1.00) * 0.20, 2)
										ELSE ROUND(t2.TARIF*ISNULL(upr.UPR,1.00)*(CASE WHEN d3_ksg.N_KSG in (select [IDKSG] from [dbo].[CalNotKus] Where tf.DATE_2 >= TBEG and tf.DATE_2 < TEND +1) THEN 1.00 ELSE ISNULL(kf.KUS,1.00) END)*ISNULL(d3_ksg.IT_SL,1.00) * 1.00, 2)
									END)
							END)
...
left join D3_DSS_OMS ds2 on tf.ID = ds2.D3_SLID and ds2.DS_TYPE in (2,3)
...
and (ksg.DLIT = 
case when cast(tf.DATE_2 as date) = cast(tf.DATE_1 as date) or datediff(day, cast(tf.DATE_1 as date), cast(tf.DATE_2 as date)) <= 3 then 1 else 0 end
or ksg.DLIT is null)
--and (ksg.DOP_KR = d3_cr.CRIT or ksg.DOP_KR is null)
and (ksg.DOP_KR in (select CRIT from D3_CRIT_OMS cr where cr.D3_KSGID = d3_ksg.ID) or ksg.DOP_KR is null)
...
join CalcKsgTarif t2 on t.ID = t2.IDKSG  and (tf.DATE_2 >= t2.DBEG and tf.DATE_2 < t2.DEND +1)
left join KSG_SOD as ksgsod on (tf.DATE_2 >= ksgsod.TarifDateStart and tf.DATE_2 < ksgsod.TarifDateEnd +1)
left join CalcUprk upr on t.ID = upr.IDKSG and (tf.DATE_2 >= upr.TBEG and tf.DATE_2 < upr.TEND +1)
...
where sl.D3_SCID
-- in (select id from d3_schet_oms where month =3) and 
=@p1 and 
tf.METOD_HMP is null -- ÑÌÎ
...
-- âûáîð ÊÑÃ ÄÑ ïî óñëóãå
UPDATE @temp_ksg_itog SET sur_ksg = 1 where usl_ok = 2 and USL is not null

Select -- zsl.zsl_id, itog.*
 --rn_itog, ID, IDKSG, KDAY, SUMV_USL SUMV, TARIF -- ÌÎ
 SLID, sl_sumv, ISNULL(CONVERT(nvarchar(250), SUMV_USL), '') com -- ÑÌÎ
 --into tempKsg201803
 From (
