...
--update zs set zs.npr_date=t.date_z_1
...
----INTO ztemp_covid_112020_1

FROM [D3_SCHET_OMS] sch                  
  inner join D3_PACIENT_OMS p on p.d3_scid=sch.id 
  and sch.year=2020 and sch.month=11-- AND sch.id  IN (1082)
...
FROM [D3_SCHET_OMS] sch                  
  inner join D3_PACIENT_OMS p on p.d3_scid=sch.id 
  and sch.year=2020 and sch.month=11-- AND sch.id  IN (1084)
  inner join D3_ZSL_OMS zs on zs.D3_PID=p.id   and zs.usl_ok in (1,2) --AND zs.id=20906231
 join D3_SL_OMS s on s.D3_ZSLID=zs.ID
) t ON t.npolis=p.npolis AND t.date_z_1<usl.DATE_IN AND usl.date_in<t.DATE_Z_2
WHERE datediff(dd,t.date_z_1,usl.DATE_IN)<=2