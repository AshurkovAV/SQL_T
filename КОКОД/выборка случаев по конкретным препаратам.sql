	...
	year,sch.month,k.name [отдедение],	kp.fam+' '+kp.im+' '+kp.ot [врач],YEAR(zsl.date_z_2) [год лечения],	sl.NHISTORY,									
	--sl.id,onkusl.id as onk_id,										
											
	p.npolis,p.fam, p.im, p.ot,p.dr,									
	--f2.nam_smok,										
	sch.year,sch.month,										
...
left join KOKOD_OTD k on k.kod_otd=sl.podr
		LEFT JOIN kokod_podr AS kp ON kp.IDDOCT=dbo.f_digit_only(iddokt) AND zsl.usl_ok=kp.usl_ok									   	
		where sa.id is null and 									
			...
			AND LekPr.REGNUM IN (001691)--пемболизумаб
			--                    (001705)--пертузумаб
			                   -- (000300)--бевацизумаб
			                   --(001554) --ниволумаб
			                   
			--in (002120,002121,000300,001705,002302,001653,000412) 
					

		
	order by 	LekPr.DATE_INJ