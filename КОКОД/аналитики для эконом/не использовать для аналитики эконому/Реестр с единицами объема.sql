...
SELECT [Тип счета],  [Страховая компания],[ед. обьём],COUNT(zslid) [количество зак сл], SUM([признак переподачи]) [из них переподано],SUM([Стоимость оказанной мед. помощи]) [Стоимость оказанной мед. помощи]
,SUM(CASE WHEN [признак переподачи]=1 THEN  [Стоимость оказанной мед. помощи] end) [Стоимость переподано]
FROM
(
Select --sum(sumv)
...
GROUP BY [Тип счета],  [Страховая компания],[ед. обьём]