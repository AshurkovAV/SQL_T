   --SELECT NAME, dkkname,COUNT(nhistory)[количество случаев],COUNT (DISTINCT npolis) [количество пациентов]
   --FROM 
    --   (
...
left join[V019] v019 on cast(v019.IDHM as int)=sl.metod_hmp  											
		...
		where sa.id is null and 									
		(									
			YEAR>=2024								
			--or 								
			--(								
			--sch.year=2021 								
			--and sch.month between 1 and 9		
			AND YEAR(zsl.DATE_Z_2)=2024 --AND MONTH(zsl.DATE_Z_2) BETWEEN 1 AND 12-- период оказания						
			--)																  
			)								
			and zsl.usl_ok in (1,2)								
			--and sch.SchetType like '%C%'								
			and sch.code_mo='460061'								
			--and zsl.NotExist is null								
			--and zsl.VIDPOM<>32								
			 	
			AND LekPr.REGNUM IN --  (000246)--	Атезолизумаб
			...
			--(000246,001691, 001554,001038,002722) -- таргет
	--in (002120,002121,000300,001705,002302,001653,000412) --имунка
			--AND kd_z>7	
			--AND YEAR(zsl.date_z_1)=2020					
			--AND code_sh  LIKE 'sh9%'								
			--AND sch.id IN 								
			--				(Select --month,				
			--				        id				
											
			--						FROM [D3_SCHET_OMS] sch  		
			--						WHERE YEAR>=2020 AND nschet LIKE 'дешевые'		
			--				)				
			--			--	AND sl.NHISTORY LIKE '5317/05317'	б			
--GROUP BY nhistory,zsl.USL_OK
	--HAVING COUNT(*)>1
	--ORDER BY sl.NHISTORY
    --   ) t
   --GROUP BY NAME,dkkname 
