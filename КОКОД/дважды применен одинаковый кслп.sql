SELECT sch.month,zsl_id,p.fam,p.im,p.ot,s.nhistory,count(idsl),count(distinct idsl)--,dsko.*,sumv
...
--where p.smo=46003--p.fam like ''
----and ksg.IT_SL=1.6
----and p.fam like ''
group by  sch.month,zsl_id,p.fam,p.im,p.ot,s.nhistory,s.id
having count(idsl)<>count(distinct idsl)