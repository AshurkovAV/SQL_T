...
from
(
select distinct sch.id AS shit,zs.ID AS zslid, s.id AS slid, ou.ID AS ouid,ksg.N_KSG,lek.code_sh,date_z_2, rslt, s.NHISTORy,COUNT(DISTINCT lek.DATE_INJ) AS kol_vved
...
FROM [D3_SCHET_OMS] sch                  
 ...
 GROUP BY sch.id,zs.ID, s.id,ou.ID,ksg.N_KSG,lek.code_sh,date_z_2, s.NHISTORY, rslt
 ) t
 LEFT JOIN SCHEME_DS_2024 spr on spr.ksg=t.n_ksg and spr.code_sh=t.code_sh
