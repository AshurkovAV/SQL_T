	...
	select DISTINCT case when ds1 between 'C00' and 'C14.Z' then 'Ãîëîâà, øåÿ'
			              when ds1  like 'C50%' then 'ÎÌÆ'
				          when ds1   between 'C18' and 'C20.Z' then 'ÎÌÆ' ds1 like 'C16%'
			...
			Ãîëîâà, øåÿ  between ‘C00’ and ‘C14.Z’ 
ÎÌÆ   like ‘C50%’ 
Ðàê ëåãêîãî like ‘C34%’ 
Ðàê æåëóäêà like ‘C16%’ 
Êîëîðåêòàëüíûé ðàê between ‘C18’ and ‘C20.Z’ 
Ãèíåêîëîãèÿ between ‘C51’ and ‘C58.Z’ 
Óðîëîãèÿ between ‘C60’ and ‘C68.Z’ 
Ìåëàíîìà between ‘C43’ and ‘C44.Z’ 
Ëèìôîìà between ‘C81’ and ‘C85.Z’ 
Êîñòè between ‘C40’ and ‘C41.Z’ 
Ñàðêîìû between ‘C45’ and ‘C49.Z’,									
	...
	sl.DS1,										
	--m.ds_name,										
	case when zsl.VIDPOM=32 then 'ÂÌÏ' ELSE 'ÊÑÃ' end names,										
...
left join[V019] v019 on cast(v019.IDHM as int)=sl.metod_hmp  											
		...
		where 							
			...
			sch.id in (select sch.id
							from  [D3_SCHET_OMS] sch   
							where YEAR=2021	
							 AND (SCH.NSCHET LIKE 'ÄÅØÅÂÛÅ'	or SCH.NSCHET LIKE '%ñäàí%'	)
						)
