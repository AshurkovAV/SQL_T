...
--при лечении онкозаболеваний стоит подозрение на онко
-- sch.id, sch.schettype,zs.ID, zs.SUMV,'' com ,s.nhistory,ksg.n_ksg,v023.NameWithID			
FROM [D3_SCHET_OMS] sch			
join D3_PACIENT_OMS p on p.d3_scid=sch.id 			
			and sch.year=2020 and sch.month=4 
			--and sch.id=@p1
...
and (v023.[N_KSG] like '%Лекарств%терап%' or v023.[N_KSG] like '%лучев%' or zs.vidpom=32 or ksg.n_ksg like 'st36.012') --точно нельзя ставить подозрение
