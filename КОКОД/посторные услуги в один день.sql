DECLARE @p1 INT=319
SELECT sch.id, zs.id, s.nhistory,usl.CODE_USL, usl.VID_VME,uname.[Name], p.npolis,usl.DATE_IN
--sch.id, zs.id, usl.CODE_USL, usl.VID_VME,ms.ishod,zs.ISHOD

FROM [D3_SCHET_OMS] sch                  
  inner join D3_PACIENT_OMS p on p.d3_scid=sch.id 
  and sch.year=2020 AND sch.id=@p1
 ...
 left join [Yamed_Spr_UslCode] uname on uname.id=usl.VID_VME
JOIN(
select usl.CODE_USL,left(usl.VID_VME,10) vid, p.npolis,usl.DATE_IN,COUNT(*) kol_vo
--sch.id, zs.id, usl.CODE_USL, usl.VID_VME,ms.ishod,zs.ISHOD
...
GROUP BY  usl.CODE_USL,left(usl.VID_VME,10),ms.ishod,zs.ISHOD, p.npolis,usl.DATE_IN
HAVING COUNT(*)>1
