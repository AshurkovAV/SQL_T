...
FROM (
		...
		WHERE dzo.D3_SCID = @pp1 AND dzo.USL_OK = 3 AND (dzo.PR_NOV = 0 OR dzo.PR_NOV IS NULL ) AND dzo.OS_SLUCH_REGION IS NULL) AS t
JOIN (
		...
		WHERE dso2.[YEAR] = 2023 AND dzo.USL_OK = 3 AND (dzo.PR_NOV = 0 OR dzo.PR_NOV IS NULL ) AND dzo.OS_SLUCH_REGION IS NULL) AS t1 
																													ON t1.FAM = t.FAM
																													AND t1.IM = t.IM
																													AND t1.OT = t.OT
																													AND t1.DR = t.DR
