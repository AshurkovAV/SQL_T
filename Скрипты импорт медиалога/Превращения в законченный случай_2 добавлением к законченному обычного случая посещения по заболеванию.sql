...
SELECT ZSL_ID, zsl.id, MAX(sl.id), MAX(p.NPOLIS), MAX(dr), MAX(LEFT(ds1, 3))ds1, MAX(sl.PROFIL), MIN(zsl.DATE_Z_1), MAX(zsl.DATE_Z_2)
FROM[D3_ZSL_OMS] zsl
JOIN [D3_SL_OMS] sl ON sl.d3_zslid = zsl.id AND sl.P_CEL25 = '3.0'
JOIN D3_pacient_OMS p ON d3_pid = p.id
WHERE zsl.D3_SCID = @D3_SCID AND OS_SLUCH_REGION IS NULL 
AND sl.PROFIL NOT IN (63,85,86,87,88,89,90) 
...
DECLARE @zsl_id INT, @sl_id INT,  @NPOLIS NVARCHAR(20), @dr DATETIME, @ds1 [nvarchar](3), @PROFIL [int]
DECLARE @datez_1 DATETIME, @datez_2 DATETIME
DECLARE vendor_cursor CURSOR FOR 
	SELECT t.ZSLG_ID, t.zsl_id, t.sl_id, t.datez_1, t.datez_2, t.NPOLIS, t.dr, t.ds1, t.PROFIL
	FROM @ztemp_301_1 AS t	

OPEN vendor_cursor
FETCH NEXT FROM vendor_cursor INTO @ZSLG_ID, @zsl_id, @sl_id, @datez_1, @datez_2, @NPOLIS,@dr,@ds1,@PROFIL
WHILE @@FETCH_STATUS = 0
BEGIN	
	INSERT INTO @ztemp_gl (ZSLG_ID,    zsl_id,   sl_id,    datez_1,     datez_2,    zsl_id29, sl_id29,  date_1,   date_2)
	EXEC p_join_visit_2 @ZSLG_ID, @zsl_id, @sl_id, @datez_1, @datez_2, @NPOLIS,@dr,@ds1,@PROFIL	
