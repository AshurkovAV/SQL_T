...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.11.2019',NULL,2,'--003K.00.0361
--Несоответствие результата обращения и исхода заболевания (летальность) RSLT={405, 406}  ISHOD=403
...
where v9.IDRMP IN (405, 406) 
AND v12.IDIZ  not IN (403)','003K.00.0361_Несоответствие результата обращения и исхода заболевания (летальность) RSLT={405, 406}  ISHOD=403','003K.00.0361_Несоответствие результата обращения и исхода заболевания (летальность) RSLT={405, 406}  ISHOD=403','Несоответствие результата обращения и исхода заболевания (летальность)',1,0.00,4,1.00,9,2,'003K.00.0361','33DD8F22-F472-40E8-B829-101BA4B42BB8')
	
	DELETE FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0360%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0361%' ) 	
	BEGIN
