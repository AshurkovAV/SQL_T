...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0630_Несоответствие сопутствующего диагноза полу пациента
...
WHERE   dss.DsType=2 
  ...
  --and a.date >''20200101''','003K.00.0630_Несоответствие сопутствующего диагноза полу пациента','003K.00.0630_Несоответствие сопутствующего диагноза полу пациента
--Ошибка фиксируется при указании диагноза  из рубрики О МКБ-10 для W=1 ','Несоответствие сопутствующего диагноза полу пациента',1,0.00,4,1.00,9,2,'003K.00.0630','82C96A8F-344C-47F2-B8DE-D013EF2F7321')
	
	DELETE FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0630%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0630%' ) 	
	BEGIN
