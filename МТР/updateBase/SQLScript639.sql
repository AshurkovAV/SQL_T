...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0620_Несоответствие диагноза полу пациента
...
--DECLARE @AccountId  INT=25571
 ...
 INNER JOIN M001 AS m ON  m.Id =e.DiagnosisGeneral --первичный
...
--and a.date >''20200101''
','003K.00.0620_Несоответствие диагноза полу пациента','003K.00.0620_Несоответствие диагноза полу пациента
-- Ошибка фиксируется при указании диагноза  из рубрики О МКБ-10 для W=1  ','Несоответствие диагноза полу пациента',1,0.00,4,1.00,9,2,'003K.00.0620','93D75F06-03CF-49E8-BEAE-E8474D152A7F')
	
	DELETE FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0620%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0620%' ) 	
	BEGIN
