...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0430_Количество койко-дней_пациенто-дней превышает длительность лечения в дневном стационаре
...
where  zfme.AssistanceConditions in (2) 
         and (isnull(Kday,0)=0 
       or 
        (datediff(dd,z.EventBegin,z.EventEnd)=0 and kday<>1
      or datediff(dd,z.EventBegin,z.EventEnd)<>0 and kday>datediff(dd,z.EventBegin,z.EventEnd)+1
      )
    )
...
','003K.00.0430_Количество койко-дней/пациенто-дней превышает длительность лечения в дневном стационаре','003K.00.0430_Количество койко-дней/пациенто-дней превышает длительность лечения в дневном стационаре','Количество койко-дней/пациенто-дней превышает длительность лечения в дневном стационаре',1,0.00,4,1.00,9,2,'003K.00.0430','0527ABC8-BE3A-4B6A-82FD-386DD1DA681F')
	
	DELETE FROM FactExpertCriterion WHERE OidCode = '0527ABC8-BE3A-4B6A-82FD-386DD1DA681F' --[Description] LIKE '003K.00.0501%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE OidCode = '0527ABC8-BE3A-4B6A-82FD-386DD1DA681F')--[Description] LIKE '003K.00.0501%' ) 	
	BEGIN
...
DELETE
FROM [ZFactSank]
WHERE [Comments] LIKE '%003K.00.0430_Количество койко-дней/пациенто-дней превышает длительность лечения в дневном стационаре%' AND EmployeeId=99999
  

EXEC [dbo].[versionUpdate] 658
COMMIT TRAN;		
END TRY
		BEGIN CATCH			
