...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0451_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=1 100<RSLT<200  01.04.2020  
...
where zsl.AssistanceConditions=1 and 
SUBSTRING(CAST(v9.IDRMP AS VARCHAR(3)), 1, 1)<>zsl.AssistanceConditions 
--order by fp.AccountId , fp.externalid','003K.00.0451_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=1 100<RSLT<200','003K.00.0451_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=1 100<RSLT<200','Взаимное несоответствие результата лечения и условий оказания медицинской помощи',1,0.00,4,1.00,9,2,'003K.00.0451','68077346-EBFC-4E7F-BCF7-6D1A1BD879DE')
	
	DELETE FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0451%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0451%' ) 	
	BEGIN
