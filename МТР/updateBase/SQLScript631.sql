...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0461_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=2 
...
where zsl.AssistanceConditions=2 and 
SUBSTRING(CAST(v9.IDRMP AS VARCHAR(3)), 1, 1)<>zsl.AssistanceConditions 
--order by fp.AccountId , fp.externalid
','003K.00.0461_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=2','003K.00.0461_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=2','Взаимное несоответствие результата лечения и условий оказания медицинской помощи',1,0.00,4,1.00,9,2,'003K.00.0461','0480641A-0CF0-4C22-A483-323737420310')
	
	DELETE FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0461%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0461%' ) 	
	BEGIN
