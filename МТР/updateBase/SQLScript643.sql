...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0440_Количество койко-дней(пациенто-дней) превышает длительность лечения в стационаре
...
where  zfme.AssistanceConditions in (1) 
      ...
      or datediff(dd,EventBeginZ1,EventEndZ2)<>0 and zfme.Kdz>datediff(dd,EventBeginZ1,EventEndZ2)
      )
    )
    
 and a.TerritoryAccountId=@AccountID ','003K.00.0440_Количество койко-дней(пациенто-дней) превышает длительность лечения в стационаре','003K.00.0440_Количество койко-дней(пациенто-дней) превышает длительность лечения в стационаре','Количество койко-дней(пациенто-дней) превышает длительность лечения в стационаре',1,0.00,4,1.00,9,2,'003K.00.0440','2296308F-A6B5-419F-89FF-11EFA067878A')
	
	DELETE FROM FactExpertCriterion WHERE OidCode = '2296308F-A6B5-419F-89FF-11EFA067878A' --[Description] LIKE '003K.00.0501%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE OidCode = '2296308F-A6B5-419F-89FF-11EFA067878A')--[Description] LIKE '003K.00.0501%' ) 	
	BEGIN
