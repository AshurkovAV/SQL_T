...
DECLARE @FactExpertCriterion TABLE(
	...
	VALUES ('',1,53,'',1,1,'01.04.2020',NULL,2,'--003K.00.0471_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=3 
...
where zsl.AssistanceConditions=3 and 
SUBSTRING(CAST(v9.IDRMP AS VARCHAR(3)), 1, 1)<>zsl.AssistanceConditions 
--order by fp.AccountId , fp.externalid','003K.00.0471_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=3 ','003K.00.0471_Взаимное несоответствие результата лечения и условий оказания медицинской помощи   USL_OK=3 ','Взаимное несоответствие результата лечения и условий оказания медицинской помощи',1,0.00,4,1.00,9,2,'003K.00.0471','5B977378-0976-46C7-926E-FA3022FC6A85')
	
	DELETE FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0471%'
	
	IF NOT EXISTS(SELECT * FROM FactExpertCriterion WHERE [Description] LIKE '003K.00.0471%' ) 	
	BEGIN
