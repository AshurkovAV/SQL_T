...
DATA_AMBULAT_PATIENT_COUPON.ED_AIDINGS_FORMS_ID FOR_POM, --Форма оказания медицинской помощи
...
ED_TREATMENT_RESULTS.ED_TREATMENT_RESULT_CODE RSLT,--Результат обращения
ED_OUTCOMES.ED_OUTCOMES_CODE ISHOD, --Исход заболевания
...
MOTCONSU_EV.KRN_GUID SL_ID, --Идентификатор случая
301 LPU_1, --Подразделение МО
...
MOTCONSU_EV.DATE_CONSULTATION DATE_1,  MOTCONSU_EV.EV_DATE_CLOSE DATE_2,
...
FROM G_EMISZKO_KGB6.dbo.FM_BILLDET
LEFT JOIN G_EMISZKO_KGB6.dbo.MEDECINS MEDECINS_SERV on FM_BILLDET.MEDECINS_ID = MEDECINS_SERV.MEDECINS_ID
JOIN G_EMISZKO_KGB6.dbo.FM_BILL on FM_BILL.FM_BILL_ID = FM_BILLDET.FM_BILL_ID
LEFT JOIN G_EMISZKO_KGB6.dbo.FM_SERV on FM_SERV.FM_SERV_ID = FM_BILLDET.FM_SERV_ID
JOIN G_EMISZKO_KGB6.dbo.MOTCONSU MOTCONSU_EV ON MOTCONSU_EV.MOTCONSU_ID = FM_BILL.MOTCONSU_ID
JOIN G_EMISZKO_KGB6.dbo.DATA_AMBULAT_PATIENT_COUPON ON DATA_AMBULAT_PATIENT_COUPON.MOTCONSU_ID = MOTCONSU_EV.MOTCONSU_ID
LEFT JOIN G_EMISZKO_KGB6.dbo.MEDDEP ON MEDDEP.MEDDEP_ID = ISNULL(DATA_AMBULAT_PATIENT_COUPON.VRACH_OTDELENIE, MOTCONSU_EV.MEDDEP_ID)
JOIN G_EMISZKO_KGB6.dbo.MEDECINS on MEDECINS.MEDECINS_ID = ISNULL(MEDDEP.MEDECINS_ID, MOTCONSU_EV.MEDECINS_ID)
JOIN PATIENTS on PATIENTS.PATIENTS_ID = MOTCONSU_EV.PATIENTS_ID
LEFT JOIN (SELECT * FROM G_EMISZKO_KGB6.dbo.DATA_INS_POLICIES_OMI WITH (NOLOCK) WHERE IS_MAIN = 1 AND DEYSTVUYUSCHIY=1) DIPO ON DIPO.PATIENTS_ID = PATIENTS.PATIENTS_ID
LEFT JOIN G_EMISZKO_KGB6.dbo.FM_ORG ORG WITH (NOLOCK) ON ORG.FM_ORG_ID = DIPO.SMO
LEFT JOIN G_EMISZKO_KGB6.dbo.DATA_DIAGNOSIS DD on DD.MOTCONSU_ID = MOTCONSU_EV.MOTCONSU_ID AND dd.VID_ZABOLEVANIQ = 0
LEFT JOIN G_EMISZKO_KGB6.dbo.CIM10 MKB on MKB.CIM10_ID = DD.SS_LKA_CIM10
LEFT JOIN G_EMISZKO_KGB6.dbo.DATA_DIAGNOSIS DD1 on DD.MOTCONSU_ID = MOTCONSU_EV.MOTCONSU_ID AND dd.VID_ZABOLEVANIQ = 2
LEFT JOIN G_EMISZKO_KGB6.dbo.CIM10 MKB1 on MKB1.CIM10_ID = DD1.SS_LKA_CIM10
LEFT JOIN G_EMISZKO_KGB6.dbo.FM_DEP on FM_DEP.FM_DEP_ID = ISNULL(MEDDEP.FM_DEP_ID, MOTCONSU_EV.FM_DEP_ID)
LEFT JOIN G_EMISZKO_KGB6.dbo.ED_TREATMENT_RESULTS ED_TREATMENT_RESULTS ON ED_TREATMENT_RESULTS.ED_TREATMENT_RESULT_ID = DATA_AMBULAT_PATIENT_COUPON.ED_TREATMENT_RESULT_ID 
LEFT JOIN G_EMISZKO_KGB6.dbo.ED_OUTCOMES ED_OUTCOMES ON ED_OUTCOMES.ED_OUTCOMES_ID = DATA_AMBULAT_PATIENT_COUPON.ED_OUTCOMES_ID
LEFT JOIN G_EMISZKO_KGB6.dbo.ED_RECEPTION_TYPES AS ert ON ert.ED_RECEPTION_TYPE_ID = DATA_AMBULAT_PATIENT_COUPON.VID_PRIEMA
JOIN @docTab doc ON doc.SNILS = MEDECINS.SNILS
WHERE MOTCONSU_EV.DATE_CONSULTATION >= '2019-04-01' AND MOTCONSU_EV.DATE_CONSULTATION <= '2019-04-30'-- and MOTCONSU_EV.EV_DATE_CLOSE <= '2019-03-31'
AND MEDECINS.MEDECINS_ID IN(2600038)--2600160
