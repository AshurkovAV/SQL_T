...
DECLARE @var1 VARCHAR(50), @script NVARCHAR (max)
        ...
        DECLARE cur CURSOR FAST_FORWARD READ_ONLY LOCAL FOR
          ...
          WHERE  
		  --name NOT IN ('G_EMISZKO_ODKB','G_EMISZKO_OPC','G_EMISZKO_OPHTALM','G_EMISZKO_SEMASHKO','G_EMISZKO_KKPB','G_EMISZKO_KOKOD') AND
		   name LIKE 'G_EMISZKO_KGB6'-- OR name LIKE 'R_EMISZKO_%' OR name LIKE 'Z_EMISZKO_%' 
        OPEN cur
...
INSERT INTO #tbl
( [МО], [Выписан от имени врача + код ДЛО], [ФИО пациента], [Снилс пациента], [Серия рецепта], [Номер рецепта], Утвержден, [Статус рецепта в МИС]
, [Статус рецепта в Аптеке], [Причина аннулирования], [Дата выдачи рецепта], [Срок действия рецепта], [Срок действия рецепта завершен?], Выписан,
Количество, Дозировка, [Отпущено в аптеке], [Отпущено ЛС], [Отпущено шт.] )
...
from DATA_GRACE_PRESCRIPTION dgp
...
where dgp.DATE_CONSULTATION >= ''2020-01-01'' and dgp.DATE_CONSULTATION <= ''2020-04-03''
