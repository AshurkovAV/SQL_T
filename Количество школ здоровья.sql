SELECT(CASE WHEN dso.[MONTH] = 1 THEN  'Январь'
		  ...
		  ELSE 'Что-то пошло не так' END) [месяц подачи] , (SELECT TOP 1 ysme.NameWithID FROM Yamed_Spr_MedicalEmployee AS ysme WHERE ysme.SNILS = iddokt)iddokt1, COUNT(*) kol 
FROM D3_SCHET_OMS AS dso
	JOIN D3_ZSL_OMS AS dzo	ON dzo.D3_SCID = dso.ID
		JOIN D3_SL_OMS AS dso2 ON dso2.D3_ZSLID = dzo.ID
WHERE dso.[YEAR] = 2025 AND dso2.P_CEL25 = '1.9'
