...
SELECT p2.NOM, p.PRENOM, p.PATRONYME, p2.NE_LE, p.DATE_START, ps.NAME
		FROM Sql_miac.R_EMISZKO_KURSKAYA.dbo.PLANNING AS p
		JOIN Sql_miac.R_EMISZKO_KURSKAYA.dbo.PATIENTS AS p2 ON p.PATIENTS_ID=p2.PATIENTS_ID
		JOIN Sql_miac.R_EMISZKO_KURSKAYA.dbo.PL_SUBJ AS ps ON p.PL_SUBJ_ID = ps.PL_SUBJ_ID
		WHERE p.KRN_CREATE_USER_ID IN(130151, 130001)
		AND CAST(p.DATE_CONS AS DATE) = @d AND p.DATE_CANCEL IS NULL
SELECT rec.*, pr.AdrOmsRegion, pr.AdrOmsRn, pr.AdrOmsNaspunkt, pr.AdrOmsUl, pr.AdrOmsDom, pr.AdrOmsKorp, pr.AdrOmsKv, pr.NameWithID
--(SELECT f.namewithid FROM medic2.dbo.f003 f WHERE f.mcod=pr.PrMo)
FROM(SELECT * FROM @ztemp_rec AS zr	) AS rec
LEFT JOIN (
	SELECT p.*, f.NameWithID
...
--WHERE pr.PrMo != 460013) AS rec
--LEFT JOIN (SELECT ISNULL([FAM],'')[FAM],ISNULL([IM], '')[IM],ISNULL([OT], '')[OT],[W],[DR],[DOCTP],[DOCS],[DOCN],CAST(DOCDT AS DATE)DOCDT,[DOCORG]
--           FROM SQL_FOMS.SRZ.[dbo].[People] p) pr1 ON rec.NOM = pr1.Fam AND rec.PRENOM = isnull(pr1.Im, '') AND rec.PATRONYME = pr1.Ot AND rec.NE_LE = pr1.dr
